...
| top_logprobs_p             | `Optional[Union[List[float], float]] = None`                                 | If return_logprob, return the smallest set of tokens whose cumulative probability ≥ this threshold (Top-P logprobs). Returns variable-length logprobs per position. Can be used together with `top_logprobs_num`. |
| return_logprobs_in_base64  | `bool = False`                                                               | If return_logprob, encode logprob arrays as base64 float32/int32 binary instead of JSON lists. Reduces response size by ~50% with negligible latency overhead.   |
...
Return the top-K most likely tokens and their log probabilities at each position:
...
# meta["output_top_logprobs"][pos] = [(logprob, token_id, token_text), ...]
print(meta["output_top_logprobs"])
```

#### Top-P logprobs

Return the smallest set of tokens whose cumulative probability ≥ `top_logprobs_p`. The number of tokens returned varies per position:
...
meta = response.json()["meta_info"]
# meta["output_top_p_logprobs"][pos] = [(logprob, token_id, token_text), ...]
# Length varies per position depending on the probability distribution
print(meta["output_top_p_logprobs"])
```

`top_logprobs_num` (Top-K) and `top_logprobs_p` (Top-P) can be used together in the same request.

To include logprobs for the input/prompt tokens as well, set `logprob_start_len=0`:
...
response = requests.post(
    ...
    json={
        ...
        "return_logprob": True,
        "top_logprobs_p": 0.9,
        "logprob_start_len": 0,   # include input tokens; default -1 (output only)
    },
)
meta = response.json()["meta_info"]
print(meta["input_top_p_logprobs"])   # per-position logprobs for input tokens
print(meta["output_top_p_logprobs"])  # per-position logprobs for output tokens
```

#### Base64-encoded logprobs

Set `return_logprobs_in_base64=True` to receive logprob arrays encoded as base64 binary instead of JSON lists. This reduces response payload by ~50% with negligible latency overhead.
...
response = requests.post(
    ...
    json={
        ...
        "top_logprobs_num": 5,
        "top_logprobs_p": 0.9,
        "return_logprobs_in_base64": True,
    },
)
meta = response.json()["meta_info"]

# Decode Top-K (fixed shape [num_positions, k])
k_vals = np.frombuffer(base64.b64decode(meta["output_top_logprobs_val_base64"]), dtype="float32")
k_idxs = np.frombuffer(base64.b64decode(meta["output_top_logprobs_idx_base64"]), dtype="int32")
k_vals = k_vals.reshape(meta["output_top_logprobs_shape"])  # [num_positions, k]
k_idxs = k_idxs.reshape(meta["output_top_logprobs_shape"])

# Decode Top-P (variable length per position)
p_vals = np.frombuffer(base64.b64decode(meta["output_top_p_logprobs_val_base64"]), dtype="float32")
p_idxs = np.frombuffer(base64.b64decode(meta["output_top_p_logprobs_idx_base64"]), dtype="int32")
lengths = meta["output_top_p_logprobs_lengths"]  # -1 = None, 0 = empty, n > 0 = n entries

offset = 0
output_top_p = []
for l in lengths:
    if l == -1:
        output_top_p.append(None)
    elif l == 0:
        output_top_p.append([])
    else:
        output_top_p.append(list(zip(p_vals[offset:offset + l], p_idxs[offset:offset + l])))
        offset += l
```
